...
btnEnter.addEventListener("mousemove", (e) => {
    ...
    console.log(`x: ${(xValue - xValueBtn) * 0.1}`);
    console.log(`y: ${(yValue - yValueBtn) * 0.1}`);
    console.log(
        `translate(${(xValue - xValueBtn) * 0.1 - 50}%,${
            (yValue - yValueBtn) * 0.1 - 50
        }%) scale(1.3)`
    );
    btnEnter.style.transform = `translate(${(xValue - xValueBtn) * 0.2 - 50}%,${
        (yValue - yValueBtn) * 0.2 - 50
    }%) scale(1.3)`;
});

btnEnter.addEventListener("mouseout", () => {
    console.log(`transform:translate(-50%,-50%) scale(1)`);
    btnEnter.style.transform = `transform:translate(-50%,-50%) scale(1)`;
});
