...
function contentBlock5_Ani(target) {
    let contentBox = document.querySelector(".contentBlock5 .contentBox");
    if (target >= 75 && target <= 100) {
        contentBox.style.transform = `translate(-50%,${-90 + (target-80) * 1.2}%)`;
    }
}

document.querySelector('#mainbody').addEventListener("scroll",(e)=>{
    let targetScroll = (e.offsetHeight);
    console.log(targetScroll);
    
});
console.log(document.querySelector('#mainbody'));
