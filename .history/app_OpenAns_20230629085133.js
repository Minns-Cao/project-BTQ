...
function ScrollAnimation(elm, distance, speed) {
    ...
    window.addEventListener("scroll", function () {
        // Vị trí scroll (tính ở bottom)
        let scrollTargetBottom = window.pageYOffset + window.innerHeight;
        // console.log(scrollTargetBottom);
        

        let startPos = 0 ;
        console.log(window.innerHeight - elm.getBoundingClientRect().top);
        let x = window.innerHeight - elm.getBoundingClientRect().top
        //giới hạn di chuyển từ 0 tới distance
        let movePos = Math.max(0, Math.min(distance, ))
        console.log();

        // elm.style.transform = `translateY(${
        //     clampedPosition - startingPosition
        // }px)`;
    });
}
// function ScrollAnimation(elm, distance, speed) {
...
//     window.addEventListener("scroll", function () {
//         let appearPosition =
//             elm.getBoundingClientRect().top - window.innerHeight;
//         let scrollPostion = window.pageYOffset + window.innerHeight;
//         let startScroll;
//         let maxDistance;
//         if ((startingPosition = 0)) {
//             let startScroll = scrollPostion;
//             let maxDistance = startScroll + distance;
//         }
//         console.log("1." + startScroll);
//         console.log("2." + maxDistance);
//         // elm.style.transform = `translateY(${
//         //     clampedPosition - startingPosition
//         // }px)`;
//         console.log(`${scrollPostion} - ${appearPosition}`);
//     });
// }
