...
function changeBackround(choose = 0) {
    ...
    if (choose === 1) {
        ...
        document.querySelector('header .menu .color-bg-menu').style.backgroundImage = `linear-gradient(
            -135deg,
            var(${dataBackground[1]}) 65%,
            transparent 100%
        );`
        console.log(dataBackground[1]);
        // document.documentElement.style.setProperty('--color-bgMenu', dataBackground[1]);
    }
}
