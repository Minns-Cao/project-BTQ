...
function changeBackroundMenu(choose = 0) {
    // 1: lùi
    // other: tiến
    if (choose === 1) {
        let last_elm = dataBackground.pop();
        ...
        document.querySelector(
            ".background"
        ).style.backgroundColor = `var(${dataBackground[1]})`;
        document.documentElement.style.setProperty('--color-bgMenu', var(${dataBackground[1]}))
    } else {
        let first_elm = dataBackground.shift();
        dataBackground.push(first_elm);
        document.querySelector(
